...
static const KeyName keynames[] = {
    ...
    {SDLK_KP_9,         "kp9"},
    {SDLK_KP_0,         "kp0"},
    /* SDL3 dropped SDLK_WWW, SDLK_MAIL, SDLK_CALCULATOR and SDLK_COMPUTER.
     * The AC_* keys below are what is left of that group. */
...
void keyboard_startText(void) {
    /* SDL3 turns text input on per window, not globally. */
    SDL_StartTextInput(graphics_getWindow());
    moduleData.textActive = true;
}

void keyboard_stopText(void) {
    SDL_StopTextInput(graphics_getWindow());
    moduleData.textActive = false;
}
