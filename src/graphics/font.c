...
static graphics_Glyph const *graphics_Font_findGlyph(graphics_Font *font, unsigned unicode) {
    ...
    for (unsigned int rowIndex = 0; rowIndex < b.rows; ++rowIndex) {
        for (unsigned int c = 0; c < b.width; ++c) {
            ...
            buf[p + 1] = 255; // G
            buf[p + 2] = 255; // B
            buf[p + 3] = row[c]; // A (glyph coverage)
        }
        row += b.pitch;
    }
...
void graphics_Font_render(graphics_Font *font, char const *text, int px, int py, float r, float sx, float sy, float ox,
    ...
    while ((cp = utf8_scan(&text))) {
        ...
        graphics_Batch_add(&moduleData.batches[glyph->textureIdx], &glyph->textureCoords, x + glyph->bearingX,
                           y - glyph->bearingY, 0, 1, 1, 0, 0, 0, 0);
